...
export class JobCardApplicationModel {
  ...
  async updateStatus(trackingId: string, status: 'pending' | 'approved' | 'rejected', jobCardId?: string, rejectionReason?: string): Promise<JobCardApplication | null> {
    const result = await this.db.query(
      'UPDATE job_card_applications SET status = $1, job_card_id = $2, rejection_reason = $3, updated_at = NOW() WHERE tracking_id = $4 RETURNING *',
      [status, jobCardId || null, rejectionReason || null, trackingId]
    );
