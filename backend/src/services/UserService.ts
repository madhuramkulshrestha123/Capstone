      ...
      const workersWithDetails = await Promise.all(users.map(async (user) => {
        try {
          ...
          return {
            ...
            bank_name: jobCard?.bank_name || null,
            account_number: jobCard?.account_number || null,
            ifsc_code: jobCard?.ifsc_code || null,
            aadhaar_linked: !!(jobCard?.account_number && jobCard?.ifsc_code),
